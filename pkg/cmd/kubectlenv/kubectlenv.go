...
// NewCmdKubectlEnv returns a new kubectl-env command. The --access-level flag
// is only consulted when linkKubeconfig is false, in symlink mode kubectl-env
// merely points KUBECONFIG at the existing session kubeconfig and Run warns if
// the flag was set anyway.
